...
OUT_ROOT="${OUT_ROOT:-/mlx_devbox/users/quyanyi/playground/AIM/logs/full7_${STRATEGY}_${TS}}"
mkdir -p "$OUT_ROOT"
...
TASKS_CSV="${TASKS_CSV:-gqa,mme,pope,scienceqa_img,docvqa_val,mmmu_val,seedbench}"
...
tasks = [t for t in os.environ.get("TASKS_CSV", "gqa,mme,pope,scienceqa_img,docvqa_val,mmmu_val,seedbench").split(",") if t]
...
for task in tasks:
    ...
    if rj:
        ...
        if task in ("gqa", "scienceqa_img", "vqav2_val"):
            ...
            for p in ("pope_accuracy", "pope_precision", "pope_recall", "pope_f1_score", "pope_yes_ratio"):
                k, v = pick_prefix(blob, p)
                rec["metrics"][p] = {"key": k, "value": v}
        elif task == "docvqa_val":
            k, v = pick_prefix(blob, "anls")
            rec["metrics"]["anls"] = {"key": k, "value": v}
        elif task == "mmbench_en_dev_nogpt":
            k, v = pick_prefix(blob, "local_eval_score")
            rec["metrics"]["local_eval_score"] = {"key": k, "value": v}
        elif task == "mmmu_val":
            k, v = pick_prefix(blob, "mmmu_acc")
            rec["metrics"]["mmmu_acc"] = {"key": k, "value": v}
        elif task == "seedbench":
            for p in ("seed_image", "seed_video", "seed_all"):
                k, v = pick_prefix(blob, p)
                if k is not None:
                    rec["metrics"][p] = {"key": k, "value": v}
