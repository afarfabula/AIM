...
OUT_ROOT="${OUT_ROOT:-/mlx_devbox/users/quyanyi/playground/AIM/logs/bench5_${STRATEGY}_${TS}}"
mkdir -p "$OUT_ROOT"
...
TASKS_CSV="${TASKS_CSV:-gqa,mme,pope,scienceqa_img,mmmu_val}"
...
tasks = [t for t in os.environ.get("TASKS_CSV", "gqa,mme,pope,scienceqa_img,mmmu_val").split(",") if t]
