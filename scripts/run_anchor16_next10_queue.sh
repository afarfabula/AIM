...
QUEUE_NAME="${QUEUE_NAME:-anchor16_next10}"
TS="$(date +%Y%m%d_%H%M%S)"
QUEUE_ROOT="${QUEUE_ROOT:-/mlx_devbox/users/quyanyi/playground/AIM/logs/ablation_queue_${QUEUE_NAME}_${TS}}"
mkdir -p "$QUEUE_ROOT"

STRATEGIES=(
  bishemethod_v2stage_anchor16_litefirst_t288
  bishemethod_v2stage_anchor16_litefirst_t192
  bishemethod_v2stage_anchor16_litefirst_t144
  bishemethod_v2stage_anchor16_litefirst_t64
  bishemethod_v2stage_anchor16_litefirst_t32
  bishemethod_v2stage_anchor16_litefirst_a3_q010
  bishemethod_v2stage_anchor16_litefirst_a3_q030
  bishemethod_v2stage_anchor16_litefirst_a3_p000
  bishemethod_v2stage_anchor16_litefirst_a3_p002
  bishemethod_v2stage_anchor16_litefirst_a3_p008
)
...
TASKS_LIST=(
  "gqa,mme,pope,scienceqa_img,mmmu_val"
  "gqa,mme,pope,scienceqa_img,mmmu_val"
  "gqa,mme,pope,scienceqa_img,mmmu_val"
  "gqa,mme,pope,scienceqa_img,mmmu_val"
  "gqa,mme,pope,scienceqa_img,mmmu_val"
...
for i in "${!STRATEGIES[@]}"; do
  ...
  label="${LABELS[$i]}"
  tasks_csv="${TASKS_LIST[$i]}"
  out_root="$QUEUE_ROOT/${idx}_${strategy}"
  mkdir -p "$out_root"

  printf '%s\t%s\t%s\t%s\n' "$idx" "$strategy" "$label" "$tasks_csv" >> "$QUEUE_ROOT/manifest.tsv"
  echo "==== QUEUE RUN idx=$idx strategy=$strategy label=$label tasks=$tasks_csv out=$out_root ===="

  set +e
  STRATEGY="$strategy" TASKS_CSV="$tasks_csv" OUT_ROOT="$out_root" "$ONE_SH" 2>&1 | tee "$out_root/queue_run.log"
  rc=${PIPESTATUS[0]}
  set -e
