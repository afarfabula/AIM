...
class LlavaMetaForCausalLM(ABC):
    ...
    def get_2dPool(self, image_feature, stride=2):
        if self.config.mm_spatial_pool_mode == "identity":
        ...
        #################### Token Merging ####################
        orig_num = image_feature.shape[1]
        image_feature = bipartite_soft_matching_merge(metric=image_feature, r=orig_num//2, x=image_feature) # 50% kept
        image_feature = bipartite_soft_matching_merge(metric=image_feature, r=orig_num//4, x=image_feature) # 25% kept
        # image_feature = bipartite_soft_matching_merge(metric=image_feature, r=orig_num//8, x=image_feature) # 12% kept
        # image_feature = bipartite_soft_matching_merge(metric=image_feature, r=orig_num//16, x=image_feature) # 6% kept
        # image_feature = bipartite_soft_matching_merge(metric=image_feature, r=orig_num//32, x=image_feature)  # 3% kept
        # image_feature = bipartite_soft_matching_merge(metric=image_feature, r=orig_num//64, x=image_feature)  # 1.5% kept

        # image_feature = bipartite_soft_matching_merge(metric=image_feature, r=24, x=image_feature) # 37.5% kept
        #################### Token Merging ####################
        return image_feature
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities=["image"], image_sizes=None):
        ...
        if type(images) is list or images.ndim == 5:
            ...
            if encoded_image_features.dim() == 3: # [B, N, C]
                ...
                for image_idx, image_feature in enumerate(image_features):
                    ...
                    if image_idx in video_idx_in_batch:  # video operations
                        ...
                        if image_aspect_ratio == "anyres" or "anyres_max" in image_aspect_ratio:
                            ...
                            image_feature = image_feature.view(num_patch_height, num_patch_width, height, width, -1) # [2, 3, 27, 27, 3584]
                        else:
                        ...
                        if "maxpool2x2" in mm_patch_merge_type:
                            ...
                            image_feature = image_feature.permute(4, 0, 2, 1, 3).contiguous()
                            image_feature = image_feature.flatten(1, 2).flatten(2, 3) # [3584, 2 * 27, 3 * 27]
                            image_feature = unpad_image(image_feature, image_sizes[image_idx]) # [3584, 48, 3 * 27]
                            c, h, w = image_feature.shape
                            times = math.sqrt(h * w / (max_num_patches * unit**2))
                            if times > 1.1:
                                image_feature = image_feature[None]
                                image_feature = nn.functional.interpolate(image_feature, [int(h // times), int(w // times)], mode="bilinear")[0]
                            image_feature = torch.cat((image_feature, self.model.image_newline[:, None, None].expand(*image_feature.shape[:-1], 1).to(image_feature.device)), dim=-1) # [3584, 48, 82]
                            image_feature = image_feature.flatten(1, 2).transpose(0, 1) # [3936, 3584]
